...
def carregar_modelo_importacao(tipo_importacao: str) -> bytes:
    """
    Carrega o modelo Excel f?sico da pasta modelos_importacao.

    Regra:
    - Prioriza os modelos novos em branco.
    - Se n?o encontrar arquivo f?sico, usa gerar_template_excel().
    - Evita baixar modelos antigos preenchidos ou exemplos.
    """
    from pathlib import Path

    tipo = str(tipo_importacao or "").strip()

    nomes_modelos = {
        "cepea_manual": "modelo_cepea_manual.xlsx",
        "ceagesp_pescados": "modelo_ceagesp_pescados.xlsx",
        "receita_expansao": "modelo_receita_vendas_expansao.xlsx",
        "mercado_privado": "modelo_scanntech_mercado_privado.xlsx",
        "curva_mercado_categoria": "modelo_curva_mercado_categoria.xlsx",
        "key_account_lojas": "modelo_key_account_lojas.xlsx",
        "compra_manual": "modelo_compra_manual.xlsx",
        "previa_vendedores": "modelo_previa_vendedores.xlsx",
    }

    candidatos = []

    nome_preferido = nomes_modelos.get(tipo)
    if nome_preferido:
        candidatos.append(Path("modelos_importacao") / nome_preferido)

    candidatos.extend([
        Path("modelos_importacao") / f"modelo_{tipo}.xlsx",
        Path("data/templates") / f"{tipo}.xlsx",
        Path("data/templates") / f"modelo_{tipo}.xlsx",
    ])
